...
(defun mpg123-ogg123-convert-frame (time filename)
  ...
  (string-to-number (mpg123:time2frame time filename)))
...
(defun mpg123:play (&optional startframe)
  ...
  (if (and mpg123-introduction-quiz-mode
	   (or (null startframe) (= (string-to-number startframe) 0)))
	  ...
	  (let ((frames (mpg123:get-music-info mpg123*cur-music-number 'frames))
		...
		(progn
		  ...
		  (if frames
		      (setq istart
			    (+ istart
			       (/ (* (string-to-number mpg123*cur-start-frame)
				     (mpg123:window-width))
				  frames))))
  ...
  (let ((p (get-buffer-process (current-buffer)))
    ...
    (if (null mpg123*cur-music-number) nil
      ...
      (setq frames (mpg123:time2frame
	    ...
	    target (/ (* (string-to-number frames) c) w))
...
(defun mpg123-mark-position ()
  ...
  (let ((f (max 0 (- mpg123*cur-playframe ;more 1.5sec back
		     (/ (string-to-number (mpg123:time2frame "00:02")) 3)))))
...
(defun mpg123-forward (arg)
  ...
  (let ()
    ...
    (if (and (looking-at "\\([0-9]+\\):\\([0-9]+\\)/\\([0-9]+\\):\\([0-9]+\\)")
	     (match-beginning 3)
	     (match-beginning 4))
	(let*((m (string-to-number (mpg123:match-string 1)))
	      (s (string-to-number (mpg123:match-string 2)))
	      (n (mpg123:get-music-number))
	      (c (current-column))
	      time M S T)
	  (setq M (string-to-number (mpg123:match-string 3))
		S (string-to-number (mpg123:match-string 4))
		T (+ (* 60 M) S))
	...
	(progn
	  ...
	  (save-excursion
		...
		(put 'mpg123:add-musiclist-to-point 'length
		     (format "%02d:%02d"
			     (string-to-number (mpg123:match-string 1))
			     (string-to-number (mpg123:match-string 2)))))
  ...
  (if mpg123-mixer-command
	...
	(let ((b (get-buffer-create " *mpg123 mixer* "))
	  ...
	  (if (re-search-forward "set to *\\([0-9]+\\):\\([0-9]+\\)" nil t)
		...
		(setq vol (cons (string-to-number left) (string-to-number right))))
	    (setq vol "unknown"))
