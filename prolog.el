  ...
  '((eclipse
    ...
    (swi
     ("discontiguous" "dynamic" "ensure_loaded" "export" "export_list" "import"
      ...
      "use_module" "volatile"))
    (yap
     ("block" "char_conversion" "compile" "consult" "discontiguous"
      "dynamic" "elif" "else" "encoding" "endif" "ensure_loaded"
      "expects_dialect" "if" "include" "initialization"
      "meta_predicate" "module" "module_transparent" "multifile" "noprofile"
      "parallel" "public" "op" "reconsult" "reexport" "sequential"
      "thread_local" "use_module" "wait"))
    (gnu
...
(defvar prolog-known-systems '(eclipse mercury sicstus swi gnu yap))
...
Uses the current case-table for extracting the relevant information."
  (let ((up_string "")
    ...
    ;;     (current-case-table))
      ...
      (let ((key 0))
        (while (< key 256)
          (let (nkey (int-to-char key))
            (cond
             ((and (eq nkey (upcase key))
                   (eq nkey (downcase key)))
              ;; Do nothing if upper and lower case are the same
              )
             ((eq nkey (downcase key))
              ;; The char is lower case
